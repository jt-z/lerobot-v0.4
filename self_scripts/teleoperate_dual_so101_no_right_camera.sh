...
check_cameras() {
    ...
    for camera in "$LEFT_HAND_CAMERA" "$LEFT_MAIN_CAMERA"; do
        if [ -e "$camera" ]; then
...
main() {
    print_header "双臂 SO-101 遥操作脚本（不使用右臂摄像头）"
    ...
    lerobot-teleoperate \
        ...
        --robot.left_arm_config.cameras="{
            ...
            top: {type: opencv, index_or_path: $LEFT_MAIN_CAMERA, width: $CAMERA_WIDTH, height: $CAMERA_HEIGHT, fps: $CAMERA_FPS}
        }" \
